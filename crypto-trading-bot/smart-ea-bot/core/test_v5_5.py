"""Test v5.5 — 200-day SMA trend following"""
import sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from core.yahoo_data_fetcher import fetch_yahoo_bars
from core.backtest_engine import BacktestEngine
from bots.trend_rider_v5_5.strategy import trend_rider_v5_5_strategy

print("Testing Trend Rider v5.5 (200-day SMA)...")
for asset in ["BTCUSD", "ETHUSD"]:
    bars = fetch_yahoo_bars(asset, interval="1d", period="5y")
    print(f"{asset}: {len(bars)} bars")
    if len(bars) > 250:
        result = BacktestEngine(10000.0).run(trend_rider_v5_5_strategy, bars, asset, fee_pct=0.10, slippage_pct=0.05)
